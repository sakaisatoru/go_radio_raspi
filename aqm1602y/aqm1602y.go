...
var (
	sevensegfontparts = []byte{
		0x40,
		0x03, 0x07, 0x07, 0x07, 0x07, 0x07, 0x03, 0x00,
		0x1F, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x1C, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1C, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x1F, 0x1F, 0x00,
		0x1F, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x1F, 0x00,
		0x03, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // ￣
		0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x03, 0x00, // ＿
		0x00, 0x00, 0x00, 0x00, 0x00, 0x1E, 0x1C, 0x00} // 　＿

	sevensegfont = [10][]byte{
		{0x40, 0, 1, 2, 0x40, 0, 3, 2},             // 0
		{0x40, 0x20, 0x20, 2, 0x40, 0x20, 0x20, 2}, // 1
		{0x40, 5, 4, 2, 0x40, 0, 3, 7},             // 2
		{0x40, 5, 4, 2, 0x40, 6, 3, 2},             // 3
		{0x40, 0, 0x20, 2, 0x40, 0x20, 1, 2},       // 4

		{0x40, 0, 4, 0x20, 0x40, 6, 3, 2},    // 5
		{0x40, 0, 4, 0x20, 0x40, 0, 3, 2},    // 6
		{0x40, 0, 1, 2, 0x40, 0x20, 0x20, 2}, // 7
		{0x40, 0, 4, 2, 0x40, 0, 3, 2},       // 8
		{0x40, 0, 4, 2, 0x40, 6, 3, 2}}       // 9
...
func (d *AQM1602Y) Setpos(x uint8) {
...
/*
 * ユーザー定義フォントで時刻を表示する
 * 扱うのは 0..9(0x30..0x39), :(0x3a) , spc(0x20)のみ
 * ! は : のブランクとして扱う
 */
func (d *AQM1602Y) ShowClockWithUserfont(x uint8, s []byte) {
	...
	for n := range s {
		...
		case 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39:
			d.Setpos(x)
			d.bus.Write(sevensegfont[s[n]-0x30][0:4])
			d.Setpos(x + 0x20)
			d.bus.Write(sevensegfont[s[n]-0x30][4:])
			x += 3
		case 0x3a:
			d.Setpos(x)
			d.bus.Write([]byte{0x40, 0xa5}) // ･ 半角中黒
			d.Setpos(x + 0x20)
			d.bus.Write([]byte{0x40, 0xa5}) // ･ 半角中黒
			x++
		case 0x20:
			if x == x0 {
				// 	時の最初の１桁が空白の場合の処理
				d.Setpos(x)
				d.bus.Write([]byte{0x40, 0x20, 0x20, 0x20})
				d.Setpos(x + 0x20)
				d.bus.Write([]byte{0x40, 0x20, 0x20, 0x20})
				x += 3
			} else {
